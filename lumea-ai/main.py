"""
Lumea AI Service — FastAPI + litellm
Deployed on Vercel (vercel.json configures ASGI adapter)
"""
from __future__ import annotations
import json
import os
import re
import time
from typing import Optional

import redis
from dotenv import load_dotenv
from fastapi import FastAPI, HTTPException, Header, Depends
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import JSONResponse
import litellm
from pydantic import BaseModel

load_dotenv()

# ── litellm model chain (Groq → OpenRouter → Cohere) ─────────────────────────
litellm.api_key = os.getenv("GROQ_API_KEY", "")
MODELS = [
    "groq/llama-3.1-70b-versatile",
    "openrouter/meta-llama/llama-3.1-70b-instruct",
    "command-r-plus",  # Cohere fallback
]

def llm_call(messages: list, max_tokens: int = 1024) -> str:
    """Try each model in cascade until one succeeds."""
    for model in MODELS:
        try:
            resp = litellm.completion(
                model=model,
                messages=messages,
                max_tokens=max_tokens,
                temperature=0.7,
                api_key=_get_api_key(model),
            )
            return resp.choices[0].message.content or ""
        except Exception as e:
            print(f"[AI] {model} failed: {e}")
    raise HTTPException(status_code=503, detail="All AI providers unavailable")

def _get_api_key(model: str) -> str:
    if "groq" in model:
        return os.getenv("GROQ_API_KEY", "")
    if "openrouter" in model:
        return os.getenv("OPENROUTER_API_KEY", "")
    return os.getenv("COHERE_API_KEY", "")

# ── Redis (Upstash HTTP) ──────────────────────────────────────────────────────
_redis_client: Optional[redis.Redis] = None

def get_redis() -> redis.Redis:
    global _redis_client
    if not _redis_client:
        url = os.getenv("UPSTASH_REDIS_URL", "redis://localhost:6379")
        _redis_client = redis.from_url(url, decode_responses=True)
    return _redis_client

# ── Rate limiting ─────────────────────────────────────────────────────────────
DAILY_LIMITS = {"generate_ideas": 2, "seo_analysis": 5, "summarize": 10, "tts": 20}

def check_rate_limit(user_id: str, action: str) -> dict:
    r = get_redis()
    key = f"ai_usage:{user_id}:{action}:{time.strftime('%Y-%m-%d')}"
    current = int(r.get(key) or 0)
    limit = DAILY_LIMITS.get(action, 5)
    return {"used": current, "limit": limit, "allowed": current < limit, "key": key}

def consume_quota(user_id: str, action: str):
    r = get_redis()
    rl = check_rate_limit(user_id, action)
    if not rl["allowed"]:
        raise HTTPException(status_code=429, detail=f"Daily {action} limit ({rl['limit']}) reached")
    pipe = r.pipeline()
    pipe.incr(rl["key"])
    pipe.expire(rl["key"], 86400)
    pipe.execute()

# ── JWT verify ────────────────────────────────────────────────────────────────
import base64
import hashlib
import hmac

def verify_api_key(x_api_key: str = Header(default="")) -> None:
    """Validate X-API-Key header. Skipped if API_KEYS env is empty (local dev)."""
    raw = os.getenv("API_KEYS", "")
    if not raw:
        return  # local dev — no check
    valid = set(raw.split("-"))
    if x_api_key not in valid:
        raise HTTPException(status_code=401, detail="Invalid or missing API key")


def verify_jwt(authorization: str = Header(...)) -> dict:
    if not authorization.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Authorization required")
    token = authorization[7:]
    parts = token.split(".")
    if len(parts) != 3:
        raise HTTPException(status_code=401, detail="Invalid token")
    h, p, s = parts
    secret = os.getenv("JWT_SECRET", "")
    sig = base64.urlsafe_b64decode(s + "==")
    expected = hmac.new(secret.encode(), f"{h}.{p}".encode(), hashlib.sha256).digest()
    if not hmac.compare_digest(sig, expected):
        raise HTTPException(status_code=401, detail="Invalid token signature")
    payload = json.loads(base64.urlsafe_b64decode(p + "=="))
    if payload.get("exp", 0) < time.time():
        raise HTTPException(status_code=401, detail="Token expired")
    return payload

# ── FastAPI app ───────────────────────────────────────────────────────────────
app = FastAPI(
    title="Lumea AI Service",
    description="AI-powered features: idea generation, SEO analysis, post summarization, and text-to-speech.",
    version="1.0.0",
    docs_url="/docs",
    openapi_url="/openapi.json",
)

app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:3000", "http://localhost:3001", "https://lumea.ink", "https://dash.lumea.ink"],
    allow_credentials=True,
    allow_methods=["GET", "POST", "OPTIONS"],
    allow_headers=["Authorization", "Content-Type"],
)

# ── Models ────────────────────────────────────────────────────────────────────
class IdeaRequest(BaseModel):
    topic: Optional[str] = None
    interests: list[str] = []
    existing_titles: list[str] = []

class SEORequest(BaseModel):
    title: str
    content: str
    tags: list[str] = []
    excerpt: Optional[str] = None

class SummarizeRequest(BaseModel):
    content: str
    max_length: int = 150

class TTSRequest(BaseModel):
    text: str
    voice: str = "alloy"  # for future OpenAI TTS integration

# ── Routes ────────────────────────────────────────────────────────────────────

@app.get("/health")
def health():
    return {"status": "ok", "service": "lumea-ai"}

@app.post("/api/ai/generate-ideas", tags=["AI"])
def generate_ideas(req: IdeaRequest, _: None = Depends(verify_api_key), user: dict = Depends(verify_jwt)):
    """Generate blog post ideas. Limit: 2/day per user."""
    consume_quota(user["user_id"], "generate_ideas")

    existing = "\n".join(f"- {t}" for t in req.existing_titles[:10])
    interests_str = ", ".join(req.interests[:10]) if req.interests else "general writing"

    prompt = f"""Generate 5 unique, compelling blog post ideas for a writer interested in: {interests_str}.
Topic hint: {req.topic or 'any relevant topic'}

Already written (avoid similar):
{existing or 'None'}

Return a JSON array of 5 objects with keys: title (string), hook (string, 1 sentence why readers will care), outline (array of 3 section headings).
Respond with ONLY the JSON array, no markdown."""

    raw = llm_call([{"role": "user", "content": prompt}], max_tokens=800)

    # Parse JSON from response
    try:
        match = re.search(r'\[[\s\S]*\]', raw)
        ideas = json.loads(match.group() if match else raw)
    except Exception:
        ideas = [{"title": raw, "hook": "", "outline": []}]

    return {"ideas": ideas[:5], "generated_at": time.strftime('%Y-%m-%dT%H:%M:%SZ')}

@app.post("/api/ai/seo-analysis", tags=["AI"])
def seo_analysis(req: SEORequest, _: None = Depends(verify_api_key), user: dict = Depends(verify_jwt)):
    """Analyse post for SEO quality. Limit: 5/day."""
    consume_quota(user["user_id"], "seo_analysis")

    # Word count from content (strip HTML)
    clean = re.sub(r'<[^>]+>', ' ', req.content)
    word_count = len(clean.split())

    prompt = f"""Analyze this blog post for SEO quality and return a JSON object with these exact keys:
- score: integer 0-100
- title_score: integer 0-100 (length 40-60 chars ideal, contains keyword)
- readability_score: integer 0-100
- keyword_suggestions: array of 5 strings (best keywords to target)
- meta_description: string (160 chars max, compelling meta description)
- improvements: array of 3 strings (specific actionable suggestions)
- word_count: integer
- estimated_read_time: integer (minutes)

Title: {req.title}
Word count: {word_count}
Tags: {', '.join(req.tags)}
Excerpt: {req.excerpt or 'None'}
Content (first 500 chars): {clean[:500]}

Respond with ONLY the JSON object."""

    raw = llm_call([{"role": "user", "content": prompt}], max_tokens=600)

    try:
        match = re.search(r'\{[\s\S]*\}', raw)
        result = json.loads(match.group() if match else raw)
    except Exception:
        result = {"score": 0, "improvements": [raw], "word_count": word_count}

    result["word_count"] = word_count
    return result

@app.post("/api/ai/summarize", tags=["AI"])
def summarize_post(req: SummarizeRequest, _: None = Depends(verify_api_key), user: dict = Depends(verify_jwt)):
    """Generate a post summary for the 'Summary' button on reading page. Limit: 10/day."""
    consume_quota(user["user_id"], "summarize")

    clean = re.sub(r'<[^>]+>', ' ', req.content).strip()
    clean = re.sub(r'\s+', ' ', clean)[:3000]  # cap input

    prompt = f"""Summarize the following blog post in {req.max_length} words or less.
Write in a clear, engaging style. Start directly with the summary, no preamble.

{clean}"""

    summary = llm_call([{"role": "user", "content": prompt}], max_tokens=300)
    return {"summary": summary.strip()}

@app.get("/api/ai/usage", tags=["AI"])
def get_usage(_: None = Depends(verify_api_key), user: dict = Depends(verify_jwt)):
    """Get the authenticated user's daily AI usage."""
    r = get_redis()
    today = time.strftime('%Y-%m-%d')
    usage = {}
    for action, limit in DAILY_LIMITS.items():
        key = f"ai_usage:{user['user_id']}:{action}:{today}"
        used = int(r.get(key) or 0)
        usage[action] = {"used": used, "limit": limit, "remaining": max(0, limit - used)}
    return {"usage": usage, "reset_at": "midnight UTC"}
